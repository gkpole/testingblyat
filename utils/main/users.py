...
class User:
    ...
    def get_bonus(self, first_name=None):
        ...
        if '@Rabbit_GameBot' in str(first_name).lower():
            bonus += bonus * 0.25
    ...
    def text(self):
        ...
        return f'👤 | Ник: <code>{self.link}</code>\n' \
               f'💸 | Баланс: <code>{to_str(self.balance)}</code>\n' \
               f'🏛 | Банк: <code>{to_str(self.bank)}</code>\n' \
               f'🌝 | D-Coin: <code>{to_str(self.coins)}</code>\n' \
               f'⏳ | Депозит: <code>{to_str(self.deposit)} {xd}</code>\n\n' \
               f'💰 | Всего: <code>{to_str(self.balance + self.bank + self.deposit)}</code>'
